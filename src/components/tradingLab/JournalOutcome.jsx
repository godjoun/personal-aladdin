...
export function JournalSnapshot({ snapshot, entryPlan }) {
  ...
  return <details className="journal-snapshot">
    ...
    {entryPlan && <details><summary>처음 저장한 시나리오 · 리스크 계획</summary><p>{entryPlan.scenarioText || '미기록'}</p><p>{entryPlan.entryReasonText}</p><p>Entry {journalNumber(entryPlan.entryPrice)} · TP {journalNumber(entryPlan.takeProfitPrice)} · SL {journalNumber(entryPlan.stopLossPrice)}</p><p>{entryPlan.leverage != null ? `${entryPlan.leverage}x` : '레버리지 미입력'} · {entryPlan.marginMode === 'ISOLATED' ? 'Isolated' : entryPlan.marginMode === 'CROSS' ? 'Cross' : '미입력'} · 증거금 {journalNumber(entryPlan.marginAmount)} · 포지션 {journalNumber(entryPlan.positionSize)} · 청산가 {journalNumber(entryPlan.liquidationPrice)}</p><p>무효화 가격 {journalNumber(entryPlan.invalidationPrice)}</p><p>{entryPlan.riskPlanText}</p><p>{entryPlan.avoidReasonText}</p></details>}
  </details>
}
