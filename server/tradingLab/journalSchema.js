...
export const TRADE_JOURNAL_COLUMNS = Object.freeze([
  ...
  ['chartLocationText', 'TEXT'],
  ['supportResistanceText', 'TEXT'],
  ['marketStructureText', 'TEXT'],
  ['trendText', 'TEXT'],
  ['volumeText', 'TEXT'],
  ['conclusionText', 'TEXT'],
  ['reasonTagsJson', 'TEXT'],
...
export function migrateTradeJournal(db) {
  db.exec(`
    CREATE TABLE IF NOT EXISTS trade_journal (
      ...
      marginMode TEXT CHECK(marginMode IS NULL OR marginMode IN ('ISOLATED', 'CROSS', 'UNKNOWN')),
      marginAmount REAL CHECK(marginAmount IS NULL OR marginAmount > 0),
      positionSize REAL CHECK(positionSize IS NULL OR positionSize > 0),
      liquidationPrice REAL CHECK(liquidationPrice IS NULL OR liquidationPrice > 0),
